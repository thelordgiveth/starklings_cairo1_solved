...
mod LizInventory {
    ...
    struct Storage {
        ...
        inventory: LegacyMap::<felt252, u32>// TODO: add storage inventory, that maps product (felt252) to stock quantity (u32)
    }
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            assert(get_caller_address() == self.contract_owner.read(), 'Contract owner is not calling');
            let a_stock = self.inventory.read(product);
            self.inventory.write(product, a_stock + new_stock);
            // TODO:
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            let p_stock = self.inventory.read(product) - quantity;
            self.inventory.write(product, p_stock)
            // TODO:
