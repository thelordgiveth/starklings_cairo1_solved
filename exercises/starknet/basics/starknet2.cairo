...
mod JillsContract {
    ...
    #[storage]
    struct Storage { 
        contract_owner: ContractAddress,// TODO: Add `contract_owner` storage, with ContractAddress type
    }

    #[constructor]
    fn constructor(
        ref self: ContractState, owner: ContractAddress
    ) { self.contract_owner.write(owner);// TODO: Write `owner` to contract_owner storage
    }

    #[external(v0)]
    impl IJillsContractImpl of super::IJillsContract<ContractState> {
        fn get_owner(self: @ContractState) -> ContractAddress { 
            self.contract_owner.read()// TODO: Read contract_owner storage
        }
    }
}
